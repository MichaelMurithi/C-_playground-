    ...
    class Program
    {
        ...
        private static async Task ProcessFile(string path)
        {
            ...
            for (var i = 0; i < lines.Length; i++)
            {
                var lineNumber = i;
                var line = lines[i];
                apiTasks.Add(Task.Run(async () => await GetWordCount(path, lineNumber, line, cancellationTokenSource)));
            }
            try
            {
        ...
        private static async Task<int> GetWordCount(string path, int lineNumber, string line, CancellationTokenSource cancellationTokenSource)
        {
            var wordCount = 0;
